...
from finites_fields import get_primitive_polynomial, \
...
class FinateFieldTest(unittest.TestCase):
    ...
    def test_build_logarithmic_table(self):
        ...
        table = build_logarithmic_table(power, primitive_polynomial)
        for i in range(number_of_elements ** 2):
            a = random.randrange(number_of_elements - 1)
            b = random.randrange(number_of_elements - 1)
            expected = divide_polynomials(multiply_polynomials(table[a], table[b]), primitive_polynomial)[1]
